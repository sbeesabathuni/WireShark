...
public class analysis_pcap_http {
  ...
  public static double entireBytes;
  public static double entireTime;
  public static void main(String args[]) {
    try {
      //PrintStream out = new PrintStream(new FileOutputStream("output_c_1080.txt"));
      //PrintStream out = new PrintStream(new FileOutputStream("output_c_1081.txt"));
      PrintStream out = new PrintStream(new FileOutputStream("output_c_1082.txt"));
      System.setOut(out);
    ...
    final Pcap pcap = Pcap.openOffline("http_1080.pcap", errbuf);
    //final Pcap pcap = Pcap.openOffline("tcp_1081.pcap", errbuf);
    //final Pcap pcap = Pcap.openOffline("tcp_1082.pcap", errbuf);
    System.out.println(pcap);
    if (pcap == null) {
      System.err.println(errbuf.toString());
      return;
    }
    final double prevTS = 0.0;
    pcap.loop(Pcap.LOOP_INFINITE, new JPacketHandler<StringBuilder>() {
      public void nextPacket(JPacket packet, StringBuilder errbuf) {
        ...
        entireBytes += totalBytes;
        entireTime += timeStamp-prevTS;
...
//        if (packetInfo.getFlag() !=2 && destinationPorts.size() == 0) {
...
//              " Seq : "+packetInfo.getSeqNumber() + " Ack : "+packetInfo.getAckNumber());
    ...
    System.out.println("Total Number of TCP Flows initiated by the sender = "+sourceMap.size());
    System.out.println("Total time taken : "+entireTime);
    System.out.println("Total number of bytes : "+entireBytes);

    //getTCPFlowDetails();
    //congestionControl();

    //comment this for tcp 1081 and 1082
    getHTTPRequestsAndResponses();
    pcap.close();
  }
  ...
  public static TCPDataPacket setTCPDataFromByteArray(byte[] packetData) {
    ...
    byte[] dataOffsetByte = Arrays.copyOfRange(packetData, tcpMap.get("dataOffset"), tcpMap.get("dataOffset") + 1);
    int dataOffset = bytetoInt(dataOffsetByte) >>> 4;
    tcpPacket.setDataOffset(dataOffset);
    ...
    int pLength = packetData.length - (dataOffset);

    byte[] payloadArray = new byte[pLength];
    //System.out.println(payloadArray.length);
    System.arraycopy(packetData, dataOffset, payloadArray, 0, pLength);
    tcpPacket.setPayload(payloadArray);
//    if (payloadArray.length > 0) {
//      tcpPacket.setParsedHTTPHeader(parseHTTPHeader(payloadArray));
//    }
    return tcpPacket;
  }
...
//    else
  ...
  public static void getHTTPRequestsAndResponses() {
    List keys = new ArrayList(sourceMap.keySet());
    for (int i=0;i<sourceMap.size();i++) {
      int currentSourcePort = (Integer)keys.get(i);
      System.out.println("currentSourcePort :"+currentSourcePort);
      List<TCPDataPacket> currentTCPList = sourceMap.get(currentSourcePort);
      for (int j = 0; j < currentTCPList.size(); j++) {
        TCPDataPacket sentPacket = currentTCPList.get(j);
        //System.out.println(receiveMap.get(currentSourcePort).size());
        if (receiveMap.get(currentSourcePort) != null) {
          for (TCPDataPacket receivedPacket : receiveMap.get(currentSourcePort)) {
            if (sentPacket.getSourcePort() == receivedPacket.getDestPort()) {
              if (sentPacket.getAckNumber() == receivedPacket.getSeqNumber()) {
                System.out.print("=====REQUEST=======");
                System.out.println("Source : "+sentPacket.getSourcePort() + " Destn : "+sentPacket.getDestPort() +
                    " Seq : "+sentPacket.getSeqNumber() + " Ack : "+sentPacket.getAckNumber());
                System.out.println("====HEADER===");
                byte[] sentPayload = sentPacket.getPayload();
                parseHTTPHeader(sentPayload);
                System.out.print("=====RESPONSE=======");
                System.out.println("Source : "+receivedPacket.getSourcePort() + " Destn : "+receivedPacket.getDestPort() +
                    " Seq : "+receivedPacket.getSeqNumber() + " Ack : "+receivedPacket.getAckNumber());
                System.out.println("====HEADER===");
                byte[] receivedPayload = receivedPacket.getPayload();
                parseHTTPHeader(receivedPayload);
